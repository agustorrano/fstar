...
// El error se debe a que no se considera el caso en que b sea 0.
...
let rec fib (x:int) : int = if x <= 1 then 1 else fib(x-2) + fib(x-1)

(* Defina un tipo 'digito' de naturales de un sólo dígito. *)
type digito = x:int{0 <= x /\ x <= 10}
...
let rec fac (x:int) : int = if x <= 0 then 1 else x * fac(x-1)
...
(* Si no pongo los casos x = 2 y x = 3 no funciona 
   fib' (x-1) >= x-1
   fib' (x-1) >= 0
   fib' (x-2) >= x-2
   fib' (x-2) >= 0
