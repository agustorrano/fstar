...
let rec not_member_all_lt (x:int) (t:bst) (y:int)
  : Lemma (requires all_lt y t /\ x >= y)
          (ensures in_tree x t = false)
= match t with
  | L -> ()
  | N (l, _, r) ->
    not_member_all_lt x l y;
    not_member_all_lt x r y

let rec not_member_all_gt (x:int) (t:bst) (y:int)
  : Lemma (requires all_gt y t /\ x <= y)
          (ensures in_tree x t = false)
= match t with
  | L -> ()
  | N (l, _, r) ->
    not_member_all_gt x l y;
    not_member_all_gt x r y
...
= match t with
  | N (L, _, _) -> ()
  | N (l, _, _) -> extract_min_preserva_all_lt y l
